...
namespace Mygame{
    ...
    public class LandModel{
        ...
        public LandModel(string land_type_string){
            //设置角色放在陆地上的位置
            positions = new Vector3[] {new Vector3(5.3F,-0.5F,0), new Vector3(6.1F,-0.5F,0), new Vector3(6.9F,-0.5F,0),
                new Vector3(7.7F,-0.5F,0), new Vector3(8.5F,-0.5F,0), new Vector3(9.3F,-0.5F,0)};
    ...
    public class BoatModel{
        ...
        /*构造函数，完成一些简单的初始化*/
        public BoatModel(){
            boat = Object.Instantiate(Resources.Load("Perfabs/Boat", typeof(GameObject)), new Vector3(4, -1.3F, 0), Quaternion.identity) as GameObject;//复制实例化对象，指定其位置并返回
            boat.name = "boat";
        ...
        public void BoatMove(){
            if (boat_sign == -1){
                ...
                move.MovePosition(new Vector3(4, -1.3F, 0));
                boat_sign = 1;
            }
            else{
                move.MovePosition(new Vector3(-4, -1.3F, 0));
            ...
            for (int i = 0; i < roles.Length; i++)
            {
                if (roles[i] != null)
                {
                    ...
                    {
                        // 如果船在起点，保持原始朝向
                        roles[i].GetRoleObject().transform.rotation = Quaternion.Euler(0, 60, 0);
                    }
                    else
                    {
                        // 如果船在终点，角色转向180度
                        roles[i].GetRoleObject().transform.rotation = Quaternion.Euler(0, -120, 0);
                    }
                }
            }
        }
        ...
        public void RemoveRoleFromBoat(RoleModel role, LandModel land)
        {
            if (land.GetLandType() == 1)
            {
                role.SetFaceToStartLand();  // 如果上岸的地方是起始岸，设置为面朝起始岸的方向
            }
            else
            {
                role.SetFaceToEndLand();    // 如果上岸的地方是终点岸，设置为面朝终点岸的方向
            }
        }
    ...
    public class RoleModel{
        ...
        public GameObject GetRoleObject()
        {
            return role;
        }

        /* 设置角色方向 */
        private void SetDirection(bool faceOriginal)
        {
            if (faceOriginal)
            {
                role.transform.rotation = Quaternion.Euler(0, -120, 0); // 面朝起始方向
            }
            else
            {
                role.transform.rotation = Quaternion.Euler(0, 120, 0); // 面朝反方向
            }
        }

        /*设置角色在起始岸的朝向*/
        public void SetFaceToStartLand()
        {
            role.transform.rotation = Quaternion.Euler(0, -120, 0);  // 面向起始岸的方向
        }

        /*设置角色在终点岸的朝向*/
        public void SetFaceToEndLand()
        {
            role.transform.rotation = Quaternion.Euler(0, 60, 0);    // 面向终点岸的方向
        }
